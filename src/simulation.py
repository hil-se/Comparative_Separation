...
def cal_comp(xt1, xf1, xt2, xf2):
    mu = (xt1+xt2) / (xt1+xt2+xf1+xf2)
    var = mu*(1-mu) / (xt1+xt2+xf1+xf2)
    return mu, var
...
def comparative_separation(x):
    ...
    mut11, vart11 = cal_comp(x["1111"], x["0111"], x["0011"], x["1011"])
    mut00, vart00 = cal_comp(x["1100"], x["0100"], x["0000"], x["1000"])
    mut10, vart10 = cal_comp(x["1110"], x["0110"], x["0001"], x["1001"])
    mut01, vart01 = cal_comp(x["1101"], x["0101"], x["0010"], x["1010"])
...
for i in range(r):
    ...
    for j in range(nc):
        if c1[j][1]==c2[j][1] or c1[j][0]==c2[j][0]:
            continue
        aij = c1[j][2]+c2[j][2]
        xij = c1[j][:2]+aij
        x.append(xij)
